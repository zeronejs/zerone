...
export function generateAstDocumentation(fileName: string): DocEntry {
    ...
    const entityClasses = classes.filter(it => it.decorators.some(decorator => decorator.name === 'Entity'));
    ...
    function getDtoImports() {
        ...
        const curUseVarNames = curVarNames.filter(curVarName => defaultValueReferences.includes(curVarName));
